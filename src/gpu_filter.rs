...
mod tests {
    ...
    #[tokio::test]
    async fn detects_known_literal_and_rejects_unrelated_data() {
        let filter = GpuFilter::init().await.unwrap();
        assert!(filter.has_any_keyword(b"OPENAI_API_KEY=sk-proj-test"));
        assert!(!filter.has_any_keyword(b"ordinary source code without credentials"));
    }
}
