...
use crate::dorks::{DorkPattern, get_github_dorks};
...
pub async fn interactive_mode() -> Result<(String, usize, usize, Vec<String>)> {
    ...
    let scan_mode = Select::new(
        "Scan mode:",
        vec!["Time-slotted (1 query)", "Full scan (10 queries)", "Google Dorks (advanced)", "Custom selection"],
    ).prompt()?;
...
pub async fn view_findings_menu() -> Result<()> {
    ...
    let action = Select::new(
        "View findings:",
        vec!["Public findings", "Private findings (full keys)", "Test API keys", "Export to CSV", "Statistics", "Back"],
    ).prompt()?;
...
fn display_public_findings(findings: &[PublicFinding]) {
    ...
    println!("{:<40} {:<30} {:<20} {:<10}", "Repository", "File", "Type", "Entropy");
    println!("{}", "-".repeat(100));
    
    for f in findings.iter().take(50) {
        let repo = truncate(&f.repository, 38);
        let file = truncate(&f.file_path.split('/').last().unwrap_or(&f.file_path), 28);
        let key_type = truncate(&f.key_type, 18);
        let entropy = f.entropy.map(|e| format!("{:.2}", e)).unwrap_or_else(|| "N/A".to_string());
...
fn display_private_findings(findings: &[PrivateFinding]) {
    ...
    if findings.len() > 20 {
        println!("... and {} more findings (use export to see all)", findings.len() - 20);
    }
}
...
async fn export_to_csv(storage: &SecureStorage) -> Result<()> {
    ...
    if include_full_keys {
        let findings = storage.load_private_findings().await?;
        let mut csv = String::from("Repository,File,Type,Full Key,Preview,Entropy,Line,Discovered\n");
...
pub fn show_dork_patterns() {
    ...
    println!("\nAvailable Google Dork Patterns\n");
    
    let mut by_category: HashMap<String, Vec<&DorkPattern>> = HashMap::new();
    for dork in &dorks {
        by_category.entry(dork.category.clone()).or_insert_with(Vec::new).push(dork);
    }
